...
class FullScrollableMatrix:
    def __init__(self, root, matrix=None, total_blocks=50, block_size_px=5):
        ...
        self.ax.set_ylim(self.total_blocks, 0)  # Инвертируем ось Y
        self.ax.set_xticks([])
        self.ax.set_yticks([])
    ...
    def on_click(self, event):
        ...
        if 0 <= row < self.total_blocks and 0 <= col < self.total_blocks:
            ...
            if self.matrix[row, col] == 1:
                print("Cannot modify blue blocks!")
                return
                
            # Меняем белый на красный и наоборот
            if self.matrix[row, col] == 0:
                self.matrix[row, col] = 2  # Белый -> Красный
            else:
                self.matrix[row, col] = 0  # Красный -> Белый
...
def take_users():
    ...
    try:
        ...
        if CONST_VAL <= 0 or isinstance(CONST_VAL,str):
            messagebox.showinfo("ошибка","Введино некоррктное число")
        ...
        messagebox.showinfo("Ошибка","Заполигте поля")
...
def matrix_window(rows,cols):
    ...
    app = FullScrollableMatrix(second_window, lake_logic.create_np_mas(int(CONST_GRID[2]),int(CONST_GRID[1]),CONST_FILE),total_blocks=800, block_size_px=5)
...
if __name__ == "__main__":
    ...
    # Ввод константы
    label_const = Label(root, text = "Введите константу от 1 и больше")
